...
public class PostController {
    ...
    public Response<List<PostVO>> getUserPosts(@RequestBody @Valid PageQuery pageQuery) {
        return null;
    }

    @GetMapping("/list/all")
    @Operation(summary = "获取所有帖子", description = "获取当前用户可见的所有帖子列表")
    public Response<List<PostVO>> getVisiblePosts(@RequestBody @Valid PageQuery pageQuery) {
        return null;
    }

    @PostMapping("/list/favorite")
    @Operation(summary = "获取关注帖子", description = "获取收藏用户的公开帖子列表")
    public Response<List<PostVO>> getFavoritePosts(@RequestBody @Valid PageQuery pageQuery) {
        return null;
    }
}
