...
export const getFollowPost = async (pageQuery: PageQuery) => {
  const res = await axios.post<BaseResponse<SimplePostData[]>>(`${BASE_URL}/post/list/favorite`, null, { params: pageQuery });
  return res.data;
};

// 创建帖子
export const createPost = async (params: AddPostRequest) => {
  const res = await axios.post<BaseResponse<PostData>>(`${BASE_URL}/post/create`, params);
  return res.data;
};

// 修改帖子
export const updatePost = async (params: PostRequest) => {
  const res = await axios.put<BaseResponse<PostData>>(`${BASE_URL}/post/update`, params);
  return res.data;
};

// 删除帖子
export const deletePost = async (id: string) => {
  const res = await axios.delete<BaseResponse<boolean>>(`${BASE_URL}/post/delete?id=${id}`);
  return res.data;
};
