...
export const addContact = async (params: AddContactRequest) => {
  const res = await axios.post<BaseResponse<ContactData[]>>(`${BASE_URL}/contact/add`, params);
  return res.data;
};

// 修改联系方式
export const updateContactById = async (params: ContactRequest) => {
  const res = await axios.put<BaseResponse<ContactData[]>>(`${BASE_URL}/contact/update`, params);
  return res.data;
};

// 删除联系方式
export const deleteContactById = async (id: string) => {
  const res = await axios.delete<BaseResponse<ContactData[]>>(`${BASE_URL}/contact/${id}`);
  return res.data;
};
